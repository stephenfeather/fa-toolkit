...
if ( true === class_exists( 'WP_CLI_Command' ) ) {
	...
	class Scrape_Product_Data_Command extends \WP_CLI_Command {
		...
		public function scrape_product_data( $args, $assoc_args ) {
			...
			$gallery_images_container = $dom->getElementById( 'gallery-container' );
			$gallery_images           = $gallery_images_container->getElementsByTagName( 'a' );
			$gallery_image_urls       = array();
			foreach ( $gallery_images as $gallery_image ) {
				$image_src            = 'https:' . $gallery_image->getElementsByTagName( 'img' )->item( 0 )->getAttribute( 'src' );
				$gallery_image_urls[] = $image_src;
			}

			// Example code to extract post content.

			$post_content_element = $dom->getElementsByTagName( 'div' );
			foreach ( $post_content_element as $element ) {
				$itemprop = $element->getAttribute( 'itemprop' );
				if ( 'description' === $itemprop ) {
					$post_content = $element->textContent; // phpcs:ignore WordPress.NamingConventions.ValidVariableName.UsedPropertyNotSnakeCase
					break;
				}
			}
			...
			$product_data = array(
				'ID'           => $product_id,
				'post_title'   => 'Case ' . $title_element->textContent, // phpcs:ignore WordPress.NamingConventions.ValidVariableName.UsedPropertyNotSnakeCase
