...
The sidecar is an output of the pass, not bookkeeping beside it. Every pass that reaches a product rewrites it from the slug set that pass actually wrote, so a slug the pass stopped writing (shadowed, denied, collided, gone from the JSON) leaves the sidecar in the same pass that removes its entry. There is no path that recomputes ownership and keeps the old list (PR #111 review).

Ownership is by slug, and that has one consequence worth stating. While a slug is in the sidecar, the entry under it is fa-toolkit's: a hand edit to an unpacked entry, or a hand-made replacement under the same slug, is overwritten the next time the product is selected, which the row marker makes the next import. To override a vendor value, deny the key first (the pass removes the entry and drops the slug), then add the local attribute; it is then foreign and never touched. Entries are not fingerprinted to detect hand edits, because a product save in wp-admin rebuilds the whole row through WooCommerce's data store, and any normalisation there (positions, value spacing) would make untouched entries look edited and orphan them. A `pa_*` entry can never be removed this way: unpacked slugs never carry the `pa_` prefix.
...
- Every pass writes all four values: row, sidecar, both markers. There is no "markers only" shortcut that could leave the sidecar behind the row. `update_post_meta()` already skips a value that has not changed, so a pass that changes nothing costs four reads and no writes. Positions are assigned in key order after the last foreign entry, and the JSON arrives key-sorted, so the same inputs give the same bytes.
